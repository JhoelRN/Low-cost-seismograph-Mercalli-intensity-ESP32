...
// A = MPU, B = WIFI, C = GPS, D = PYTHON COMMUNICATION, E = GPS MAP HTML, Z = PROCESS INTENSIDAD
...
//******
    ...
    /*
     * Se reconoce un umbral y luego se almacenan 50 lecturas para sacar la lectura de amplitud maxima
     */
  
  if(aprom>1){//tenemos que aproximar calibrar a maximo 0.1!
    estado = true;
  }
  else{
    estado = false;
...
if(estado==true){// si existe cambio

  Serial.println("Se reconocio umbral limite");//se activa comparación y cálculo de intensidad

    registros[i] = aprom;
    Serial.println(i);
    Serial.println(registros[i]);
    Serial.println(registros[0]);
    Serial.println(registros[1]);
    Serial.println(registros[2]);
    Serial.println(registros[3]);
    Serial.println(registros[40]);
    Serial.println(registros[50]);
  ...
  /*if(i = 50){//cuando ya se haya completado el "registro" de muestras
    Serial.println("Se completo el registro");
    //intensMerc = processMercalli(aprom); 
  }
*/
  estado = false;
  if(i==50){
    ...
    //intensiMerca = processMercalli(aprom)
    float maxVal = 0;
    ...
    Serial.println(maxVal);//its ok


    intensMerc = processMercalli(maxVal);
...
////////////////////////////////
                ...
                client.print("<hr> <h3>Último Reporte de Intensidad:</h3>");
                client.print(intensMerc);
...
aprom = sqrt(pow(ax_m_s2_c,2)+ pow(ay_m_s2_c,2) + pow(az_m_s2_c-g,2) );
...
*/
  ...
  Serial.print(","); 
  Serial.print(az_m_s2_c);
  Serial.print(","); 
  Serial.println(aprom);
...
//Z
int processMercalli(float valorMaximo){

  if(valorMaximo<0.0017){
